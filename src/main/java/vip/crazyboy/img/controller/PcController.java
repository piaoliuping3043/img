...
public class PcController {
    ...
    public String getArticleList(String type, @RequestParam(defaultValue = "1") Integer pageNum, Model model){
        ...
        if (StringUtils.isNotBlank(type) || null != type) {
            if (null != po && po.size() >= CommonUtil.NUM_PAGE){
                model.addAttribute("nextPageUrl", "/pc/?type="+type+"&pageNum="+(pageNum+1));
            }
            if (pageNum > 1){
                model.addAttribute("beforePageUrl", "/pc/?type="+type+"&pageNum="+(pageNum-1));
            }
        }else {
            if (null != po && po.size() >= CommonUtil.NUM_PAGE){
                model.addAttribute("nextPageUrl", "/pc/?pageNum="+(pageNum+1));
            }
            if (pageNum > 1){
                model.addAttribute("beforePageUrl", "/pc/?pageNum="+(pageNum-1));
            }
        }
        publicVar(model, po);
